...
static u32 count_bytes_in_trace(afl_forkserver_t *fsrv) {
...
static u32 count_non_255_bytes_in_trace(afl_forkserver_t *fsrv) {
...
static void dump_coverage() {
  ...
  printf("dump coverage here (%d): %0.02lf = (%u / %u)!\n",
      fsrv->child_pid,
      t_byte_ratio, t_bytes, fsrv->real_map_size);
  fflush(stdout);
}

static void signal_coverage(int signum) {
  if (signum == SIGUSR1) {
    dump_coverage();
...
static void showmap_run_target(afl_forkserver_t *fsrv, char **argv) {
  ...
  if (!fsrv->child_pid) {
    ...
    *(u32 *)fsrv->trace_bits = EXEC_FAIL_SIG;
    ...
    printf("Process %d executes child %d\n", getpid(), fsrv->child_pid);
    fflush(stdout);
    signal(SIGUSR1, signal_coverage);
    signal(SIGUSR2, signal_coverage);
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  virgin_bits = ck_realloc(virgin_bits, map_size);
